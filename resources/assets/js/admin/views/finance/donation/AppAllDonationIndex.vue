    ...
    <div v-show="loaded">
      <div v-if="itemsData.length == 0">
        <b-alert show variant="info">
          <h4 class="alert-heading">Belum Ada Transaksi Donatur</h4>
          <p>
            Masih belum ada transasksi dari donatur untuk periode ini
          </p>
          <hr>
          <p class="mb-0">
            Silahkan reminder para donatur untuk mengirimkan bukti transfer dengan membuka link yang dikirimkan melalui email yang terdaftar atau team collection officer untuk melakukan input data jika donatur mengirimkan bukti transfer tidak melalui sistem.
          </p>
        </b-alert>
      </div>
      <div v-if="itemsData.length !== 0">
        <div>
          <b-alert show variant="info" dismissible>
            <!-- <h4 class="alert-heading">Belum Ada Transaksi Donatur</h4> -->
            <p class="mb-0">
              Sistem akan mengirimkan noitifikasi secara otomatis pada tanggal <b>25 dan 3 setiap bulan</b>. Silahkan reminder para donatur setelah tanggal tersebut untuk mengirimkan bukti transfer dengan membuka link yang dikirimkan melalui email yang terdaftar atau team collection officer untuk melakukan input data jika donatur mengirimkan bukti transfer tidak melalui sistem.
            </p>
          </b-alert>
        </div>
        <b-row>
        ...
        <div style="overflow-y:auto">
          <b-table stacked="sm" stack small :fields="FieldTableItems" :items="filteredItemsData" thead-class="thead-light" :sort-by.sync="querySortBy" :sort-desc.sync="querySortDesc" @sort-changed="sortingChanged" :current-page="currentPage" :per-page="perPage">
            ...
            <template v-slot:cell(invoice_no)="data">
              <invoice :invoiceNo="data.item.invoice_no" :dataId="data.item.id" :index="data.index" :hasInvoice="data.item.has_invoice" :userId="data.item.donor.id" @hasInvoice="data.item.has_invoice = $event" />
            </template>
            ...
            <template v-slot:cell(actions)="data">
              <b-btn-group>
                <b-btn variant="primary" size="sm" v-b-tooltip.hover="'Edit'" @click="
                ...
                " variant="success" size="sm" v-b-tooltip.hover="'Send Payment Receipt'" @click="
                  triggerConfirmModal(
                    ...
                    'sendInvoice',
                    {
                      ...
                      index: data.index + (currentPage - 1) * perPage
                    }
                  )
                "><i class="fa fa-send"></i>
                </b-btn>
                <b-btn v-if="permission(5)" variant="danger" size="sm" v-b-tooltip.hover="'Delete'" @click="
                  triggerConfirmModal(
                    'Delete Transaction',
        ...
        <b-modal :title="transactionModalTitle" :no-close-on-esc="true" :hide-header-close="false" :no-close-on-backdrop="false" @hidden="handleHiddenModal" size="md" v-model="transactionModal" :ok-disabled="!(permission(3) || permission(4))" @ok="sendTransactionData">
          <b-form-group :invalid-feedback="errors.trx_date" :state="stateTrxDate">
            <b-input-group>
              ...
              <flat-pickr placeholder="Transaction Date" id="date" v-b-tooltip.hover title="Click To Edit" class="form-control" v-bind:class="{
                'is-invalid': stateTrxDate == false,
                'is-valid': stateTrxDate == true
          ...
          <b-form-group :invalid-feedback="errors.amount" :state="stateAmount">
            <b-input-group>
              ...
              <b-input type="number" class="form-control" placeholder="Transaction Amount" v-model="input.amount" :state="stateAmount" :disabled="!permission(3)" />
            </b-input-group>
          </b-form-group>
          <b-form-group v-if="transactionModalState == 'editTransaction'" :invalid-feedback="errors.verification" :state="stateVerification">
            <b-input-group>
              ...
              ]" v-model="input.verification" :state="stateVerification" :disabled="!permission(4)">
                <template slot="first">
            ...
            <b-input-group>
              ...
              <b-input type="text" :disabled="
                ...
                filteredItemsData[transactionTableIndex].has_invoice
                  ? true
                  : false
              " class="form-control" placeholder="Invoice Number (If Ready)" v-model="input.invoice_no" :state="stateInvoiceNumber" />
            </b-input-group>
          </b-form-group>
        </b-modal>
  ...
  export default {
    ...
    methods: {
      ...
      handleEditTransaction(index) {
        ...
        (this.input.status_invoice = this.filteredItemsData[
          index
        ].status_invoice);
