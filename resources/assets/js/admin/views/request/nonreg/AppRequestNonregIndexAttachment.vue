<template>
  <div>
    <p>Sebelum memulai pendaftaran beasiswa, mohon upload form yang diperlukan (Jika ada)</p>
    ...
    <small>
      <b>Nb : Jika sudah ada awardee beasiswa yang mendaftar, form yang sudah diupload tidak bisa diganti!</b>
    </small>
  </div>
</template>
<script>
  export default {
    ...
    methods: {
      ...
      handleClickDownload(filename) {
        window.open(
          `api/admin/scholarship/attachment/download?scholarshipId=${this.$route.params.scholarshipId}&filename=${filename}`,
      ...
      handleClickDelete(filename) {
        axios.delete(`api/admin/scholarship/attachment?scholarshipId=${this.$route.params.scholarshipId}&filename=${filename}`)
          .then((response) => {
      ...
      getAttachment() {
        axios.get(`api/admin/scholarship/attachment`, {
      ...
      handleFileUpload() {
        ...
        axios.post(`api/admin/scholarship/attachment`, formData, {
            params: {
              scholarshipId: this.$route.params.scholarshipId,
            },
            headers: {
