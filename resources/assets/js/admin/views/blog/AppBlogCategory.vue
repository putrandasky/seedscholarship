<template>
  <slide-y-up-transition>
    <restricted-area v-if="!permission(17)" />
  <b-card v-show="loaded"  v-if="permission(17)" >
...
export default {
  ...
  created() {
    ...
    this.$store.dispatch('storeBreadcrumbData', {
      linkBackButton: ``,
      currentPageName: `Blog Categories`
    });
  },
  computed: {
    dragOptions() {
  ...
  methods: {
    ...
    reorderData() {
      this.$store.dispatch('stateLoading', true);
      axios
        .post(`api/blog-category/reordering`, {
          data: this.data
        })
        .then(response => {
          console.log(response.data);
    ...
    updateData() {
      this.$store.dispatch('stateLoading', true);
      axios
        .patch(`api/blog-category/${this.confirmModalTempValue.id}`, {
    ...
    deleteData() {
      ...
      this.$store.dispatch('stateLoading', true);
      axios
        .delete(`api/blog-category/${this.confirmModalTempValue}`)
        .then(response => {
          console.log(response.data);
          this.getData();
          this.confirmModalTempValue = '';
