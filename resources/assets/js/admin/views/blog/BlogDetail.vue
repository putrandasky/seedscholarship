<template>
  <b-row v-show="loaded" class="d-flex">
    <b-col md="9">
      <b-card>
        ...
        <b-form-group label="Slug (Required)" label-for="slug" :invalid-feedback="errors.slug" :state="stateTitle">
          <b-form-input required :state="stateSlug" id="slug" type="text" placeholder="Input slug of your post" v-model="data.slug"></b-form-input>
        </b-form-group>
        <b-form-group label="Title (Required)" label-for="title" :invalid-feedback="errors.title" :state="stateTitle">
          <b-form-input autocomplete="off" required :state="stateTitle" id="title" type="text" placeholder="Input title of your post" v-model="data.title"></b-form-input>
        </b-form-group>
        <b-form-group label="Summary (Required for Meta Tags)" label-for="title" :invalid-feedback="errors.summary" :state="stateSummary">
          <b-form-input autocomplete="off" required :state="stateSummary" id="summary" type="text" placeholder="Input summary of your post" v-model="data.summary"></b-form-input>
        </b-form-group>
        <b-form-group ref="containerLoading" label="Description (Required but not mandatory)" label-for="body">
    ...
    <b-col md="3" class="">
      <b-card>
        ...
        <strong>Last Update</strong> : {{ data.moderations[0].created_at
        }}<br />
        <strong>Created</strong> : {{ data.created_at }}

        <div slot="footer" v-if="publishingPermition">
      ...
      <b-card>
        ...
        <v-select plain placeholder="Select Tags (Optional)" :value="data.tags" :options="tagOptions" transition="fade-select" multiple label="name"></v-select>
      </b-card>
      <b-card no-body>
        <div slot="header" class="text-center">
        ...
        <b-btn v-show="data.cover_image || new_cover_image" class="btn--corner" variant="danger" size="sm" style="top:47px" @click="handleRemoveCoverImage">
          <i class="fa fa-close"></i>
        </b-btn>
        <b-btn v-show="old_cover_image && new_cover_image" class="btn--corner" variant="warning" size="sm" style="top:47px;right:22px" @click="handleUndoCoverImage">
    ...
    <b-modal title="Leave Moderation Message" v-model="moderationModal" @ok="
        trigerConfirmModal(
      ...
      <b-form-group label-for="message" class="my-1">
        <b-form-input id="message" type="text" name="message" placeholder="Please leave a message why change this blog" v-model="new_moderations.message" />
      </b-form-group>
    </b-modal>
    <b-modal :no-close-on-esc="true" :hide-header-close="true" :no-close-on-backdrop="true" :title="confirmModalTitle" v-model="confirmModal" @ok="onConfirmModal" @cancel="onCancelConfirmModal">
      {{ confirmModalBody }}
    </b-modal>
  </b-row>
</template>
<script>
  ...
  export default {
    ...
    computed: {
      publishingPermition() {
        return this.$store.state.user.id == this.data.authorable.id || this.permission(16) ? true : false
      },
      url() {
        return this.new_cover_image ?
          this.objectUrl :
          this.data.cover_image ?
          `/storage/blog/${this.data.id}/cover/${this.data.cover_image}` :
          '/images/default-blog-cover.jpg';
      },
      stateSlug() {
        return this.errors.slug == 'no-error' ?
          true :
          this.errors.slug ?
          false :
          null;
      ...
      stateCategory() {
        return this.errors.category == 'no-error' ?
          true :
    ...
    methods: {
      ...
      postData() {
        ...
        axios
          ...
          .then(response => {
            ...
            if (
              self.old_cover_image &&
              !self.data.cover_image &&
              !self.new_cover_image
            ) {
              console.log('delete');
      ...
      updateCover(blogId) {
        ...
        formData.append('file', this.file);
        axios
          .post(`api/file/blog-cover-image/update/${blogId}`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data'
            },
            onUploadProgress: function(progressEvent) {
      ...
      deleteCover(blogId) {
        let self = this;
        axios
          .delete(`api/file/blog-cover-image/${blogId}`)
          .then(response => {
            console.log(response.data);
            this.$snotify.success(response.data.message, 'SUCCESS');
      ...
      deleteData() {
        let self = this;
        axios
          .delete(`api/blog/${this.confirmModalTempValue}`)
          .then(response => {
            console.log(response.data);
            this.$snotify.success(response.data.message, 'SUCCESS');
            self.$router.push({
