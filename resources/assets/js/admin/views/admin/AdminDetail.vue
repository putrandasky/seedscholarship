...
<script>
  export default {
    ...
    computed: {
      url() {
        return this.new_profile_picture ? this.objectUrl :
          this.data.photo ? `/storage/user/${this.data.id}/profile-picture/${this.data.photo}` :
    ...
    methods: {
      ...
      uploadCover(blogId) {
        ...
        axios.post(`api/file/admin-cover-image/${blogId}`, formData, {
            headers: {
      ...
      updateCover(blogId) {
        ...
        let self = this
        formData.append('file', this.file)
        axios.post(`api/file/admin-cover-image/update/${blogId}`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data'
            },
            onUploadProgress: function (progressEvent) {
