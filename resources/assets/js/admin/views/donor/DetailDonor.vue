    ...
    <b-row v-if="loaded">
      <b-col cols="12">
        <b-card>
          <div slot="header" class=" d-sm-flex w-sm-100 justify-content-sm-between">
            <!-- <router-link
              ...
              :to="{
                ...
                params: { periodYear: $route.params.periodYear }
              }"
            >
          ...
          <b-row>
            <b-col md="3" sm="6" class="pb-3">
              <div class="border-bottom">
                <h5>
                  ...
                  <b-button v-if="permission(11)" class="btn--corner-15 btn--xs" size="sm" variant="success" @click="
                      triggerEditProfileModal(
                        'Edit Full Name',
                        'name',
                        'text',
                        data.donor.name
            ...
            <b-col md="3" sm="6" class="pb-3">
              ...
              <span style="font-size:large;padding-left:20px">{{
                data.donor.college_department.department
              }}</span>
            </b-col>
            <b-col md="3" sm="6" class="pb-3">
              <div class="border-bottom">
        ...
        <b-card>
          ...
          <b-row>
            ...
            <b-col md="3" sm="6" class="pb-3" v-if="data.donation_category == 'AKTIF'">
              <div class="border-bottom">
                <h5><i class="fa fa-dollar"></i> Amount Plan</h5>
              </div>
              <!-- <span style="font-size:large;padding-left:20px">Rp. {{data.amount | currency}} / year</span><br> -->
              <!-- <span style="font-size:large;padding-left:20px">Rp. {{data.amount /12 | currency}} /
                month</span> -->
              <span style="font-size:large;padding-left:20px">Rp. {{ (data.amount / durationPeriod) | currency }} /
                month</span>
            </b-col>
            <b-col md="3" sm="6" class="pb-3" v-if="data.donation_category == 'PASIF'">
              <div class="border-bottom">
                <h5><i class="fa fa-dollar"></i> Amount Plan</h5>
              </div>
              <span style="font-size:large;padding-left:20px">Rp. {{ data.amount | currency }} /
                period</span>
            </b-col>
            <b-col md="3" sm="6" class="pb-3">
              <div class="border-bottom">
                <h5><i class="fa fa-file-o"></i> Contract Agreed</h5>
              </div>
              <span style="font-size:large;padding-left:20px">{{ data.is_contract_agreed }}
                <b-button @click="seeContract" v-if="data.is_contract_agreed == 'AGREED'" class="btn--corner-15 btn--xs" size="sm" variant="warning" v-b-tooltip.hover="'See Contract'"><i class="fa fa-search"></i></b-button>
                <b-button @click="resendEmailPostRegister" v-if="data.is_contract_agreed !== 'AGREED'" class="btn--corner-15 btn--xs" size="sm" variant="success" v-b-tooltip.hover="'Resend Email Confirmation'"><i class="fa fa-send"></i></b-button>
              </span>
            </b-col>
            <b-col md="3" sm="6" class="pb-3">
              <div class="border-bottom">
                <h5><i class="fa fa-file-o"></i> Contract Number</h5>
              </div>
              <span style="font-size:large;padding-left:20px">{{ data.contract_number }}
              </span>
            </b-col>
            <b-col md="3" sm="6" class="pb-3">
              <div class="border-bottom">
            ...
            <collection-officer :dataProps="data.pco" />
            <pr-officer :dataProps="data.pr" />
          </b-row>
        </b-card>
      </b-col>
      <b-col sm="12">
      ...
      <b-modal :no-close-on-esc="true" :hide-header-close="true" :no-close-on-backdrop="true" :title="editProfileModalTitle" v-model="editProfileModal" @ok="onConfirmEditProfileModal">
        <b-form-group :invalid-feedback="selectedEditProfileData.error" :state="selectedEditProfileData.state">
          <b-input :type="editProfileTypeModal" class="form-control" placeholder="Please Input Data" v-model="selectedEditProfileData.input" :state="selectedEditProfileData.state" />
        </b-form-group>
      </b-modal>
    </b-row>
  </slide-y-up-transition>
  ...
  export default {
    ...
    created() {
      ...
      this.$store.dispatch("storeBreadcrumbData", {
        ...
        currentPageName: `Detail Donatur`
      });
    },
    computed: {},
    methods: {
      ...
      onConfirmEditProfileModal() {
        axios
          .patch(`api/user-donor/update-single/${this.$route.params.userId}`, {
            value: this.selectedEditProfileData.input,
            key: this.selectedEditProfileData.key
          })
          .then(response => {
            console.log(response.data);
            this.data.donor[
              this.selectedEditProfileData.key
            ] = this.selectedEditProfileData.input;
      ...
      resendEmailPostRegister() {
        ...
        axios
          .post(
            `api/auth/donor/register/email-post-register/${this.$route.params.userId}`, {
              periodId: this.data.period_id
            }
          )
          .then(response => {
