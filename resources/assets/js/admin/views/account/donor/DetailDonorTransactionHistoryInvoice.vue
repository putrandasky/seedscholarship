<template>
  <div>
    <template v-if="invoiceNo && !hasInvoice">
      <span @click="createInvoice"><i class="fa fa-refresh" :class="{'fa-spin':isDisabled}" v-b-tooltip.hover="'Create Invoice'"
    ...
    <template v-if="hasInvoice">
      <span @click="deleteInvoice">
        <i class="fa fa-trash" v-b-tooltip.hover="'Delete Invoice'" style="color:red;cursor:pointer"></i>
      </span>
  ...
  export default {
    ...
    methods: {
      ...
      createInvoice(e) {
        ...
        axios.post(`api/file/donor-transaction/invoice`, {
      ...
      openInvoice(){
        let self = this
        window.open(
          `/api/file/donor-transaction/invoice/${this.$route.params.userId}?id=${this.dataId}&periodYear=${this.$route.params.periodYear}&invoice_no=${this.invoiceNo}`,
          self.invoice_no,
